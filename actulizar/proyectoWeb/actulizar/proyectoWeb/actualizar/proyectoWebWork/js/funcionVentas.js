...
function creaform(inc){
  ...
  chnselect.setAttribute('onchange',"enlistar('drinkList" +inc +"','#drinktable" + inc + "')");
  ...
  chnslcsnk.setAttribute('onchange',"enlistar('snacklist" +inc +"','#tablesnack" + inc + "')");
...
function enlistar(selectid, tableid){
  ...
  document.getElementById(tr.id).innerHTML= "<td id='column" + aux+1 +"'></td> <td class='precio' id ='column"+ aux+2 +"'></td> <td id='column"+ aux+3 +"'></td>"
 
  document.getElementById('column' +aux+1).innerHTML= entrada;
  document.getElementById('column' + aux+2).innerHTML= '1000' +aux;
...
function totales(){
  let suma = new Number();
   let totale = document.querySelectorAll('.precio');
   for (t = 0; t < totale.length; t++) {
    alert(typeof parseInt(totale[t].innerHTML)) ;
...
function multi(event){
  ...
  let sum2 = parseInt(event.target.parentElement.previousElementSibling.innerHTML);
   sum3 = sum1 * sum2;
